...
def generate_map():
    # 1. Setup grid matching extent: (90, 146, 8, 42)
    ...
    # 2. Batch fetch Mean Sea Level Pressure from Open-Meteo
    ...
    # 3. Interpolate pressure onto smooth 2D mesh
    interp_lon, interp_lat = np.meshgrid(np.linspace(90, 146, 250), np.linspace(8, 42, 250))
    grid_pressure_2d = griddata(
        (flat_lons, flat_lats), grid_pressures, (interp_lon, interp_lat), method='cubic'
    )
    ...
    highs = find_extrema_coords(grid_pressure_2d, interp_lon, interp_lat, mode='max', n=2)
    lows = find_extrema_coords(grid_pressure_2d, interp_lon, interp_lat, mode='min', n=2)

    # 4. Render Weather Chart to Buffer
